...
public class CamusJob extends Configured implements Tool {
    ...
    public static void populateConf(Properties props, Configuration conf, Logger log) throws IOException {
        ...
        if (hadoopCacheJarDir != null) {
            Path hadoopCacheJarDirPath = new Path(hadoopCacheJarDir);
            FileSystem fs = hadoopCacheJarDirPath.getFileSystem(conf);
            FileStatus[] status = fs.listStatus(hadoopCacheJarDirPath);
        ...
        if (externalJarList != null) {
            ...
            for (String jarFile : jarFiles) {
                ...
                if (!filterMatch) {
                    ...
                    DistributedCache.addFileToClassPath(jarFilePath, conf, jarFilePath.getFileSystem(conf));
                }
            }
        }
    }
    ...
    public void run(Class<? extends InputFormat> inputFormatClass,
        ...
        FileSystem fs = execBasePath.getFileSystem(job.getConfiguration());
