...
class OffsetInfo implements WritableComparable<OffsetInfo> {
	...
	OffsetInfo(OffsetInfo other) {
		this.set(other.topic, other.partition, other.latestOffset);
	}
...
public class SnapshotDailyMetadata {
	...
	public static void main(String[] args) throws URISyntaxException,
		...
		if (args.length != 1) {
			System.out.println("Usage: SnapshotDailyMetadata <properties-file>");
			System.exit(1);
		}
		...
		System.out.println("got " + topicMetadataList.size()
						+ " topic metadata list: " + StringUtils.join(",", topicMetadataList));
	...
	public static void writeToJsonFile(JSONArray data) throws IOException {
		Job job = new Job();
	...
	public static String getMetadataDir() throws IOException {
	...
	public static Path getOffsetsDiffPath() throws IOException {
		return new Path(getMetadataDir(), "current/difference.seq");
	}

	public static Path getCurrentOffsetsDir() throws IOException {
		return new Path(getMetadataDir(), "current");
	}

	public static Path getCurrentOffsetsPath() throws IOException {
		return new Path(getCurrentOffsetsDir(), "offsets.seq");
	}

	public static Path getOldOffsetsDir() throws IOException {
		return new Path(getMetadataDir(), "previous");
	}

	public static Path getOldOffsetsPath() throws IOException {
			...
			if (Pattern.matches(regex, mdata.topic())) {
				...
				for (PartitionMetadata pdata : mdata.partitionsMetadata()) {
					LeaderInfo leader = new LeaderInfo(new URI("tcp://"
									+ pdata.leader().getConnectionString()),
									pdata.leader().id());
		...
		for (LeaderInfo leader : offsetRequestInfo.keySet()) {
			SimpleConsumer consumer = new SimpleConsumer(leader.getUri()
							...
							Integer.parseInt(props.getProperty(
											CamusJob.KAFKA_TIMEOUT_VALUE,
											"30000")),
							Integer.parseInt(props.getProperty(
											CamusJob.KAFKA_FETCH_BUFFER_SIZE,
											"83886080")), props.getProperty(
											CamusJob.KAFKA_CLIENT_NAME,
											"KafkaMetadatFetcher"));
