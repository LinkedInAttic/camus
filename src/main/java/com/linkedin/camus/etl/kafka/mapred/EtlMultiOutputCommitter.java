...
public class EtlMultiOutputCommitter extends FileOutputCommitter {
	...
	public void addCounts(EtlKey key) throws IOException {
		String workingFileName = EtlMultiOutputFormat.getWorkingFileName(
				context, key);
	...
	public void commitTask(TaskAttemptContext context) throws IOException {
			...
			for (FileStatus f : fs.listStatus(workPath)) {
				...
				if (file.startsWith("data")) {
					...
					String partitionedFile = getPartitionedPath(context, file,
							count.getEventCount(), count.getLastKey()
									.getOffset());
		...
		SequenceFile.Writer offsetWriter = SequenceFile.createWriter(
				...
				new Path(super.getWorkPath(), EtlMultiOutputFormat
	...
	protected void commitFile(JobContext job, Path source, Path target)
			throws IOException {
	...
	public String getPartitionedPath(JobContext context, String file,
		...
		String partitionedPath = EtlMultiOutputFormat.getPartitioner(context,
				topic).generatePartitionedPath(context, topic, leaderId,
				Integer.parseInt(partition), encodedPartition);

		return partitionedPath + "/" + topic + "." + leaderId + "." + partition
				+ "." + count + "." + offset + "." + encodedPartition
				+ recordWriterProvider.getFilenameExtension();
	}
}
